...
void main() {
  IntegrationTestWidgetsFlutterBinding
      .ensureInitialized(); // Integration test 바인딩 초기화
  ...
  List<Map<String, dynamic>>? sortJsonList(dynamic data, String key) {
    ...
    list.sort((a, b) =>
        (a[key] as Comparable?)?.compareTo(b[key] as Comparable? ?? '') ?? 0);
    return list;
  }
  ...
  setUpAll(() async {
    ...
    if (prodSupabaseUrl == null ||
        prodSupabaseUrl.isEmpty ||
        prodSupabaseAnonKey == null ||
        prodSupabaseAnonKey.isEmpty ||
        testSupabaseUrl == null ||
        testSupabaseUrl.isEmpty ||
        testSupabaseAnonKey == null ||
        testSupabaseAnonKey.isEmpty) {
      throw Exception(
          'Supabase credentials environment variables are not set or empty in .env file.');
    }
  ...
  group('Supabase Configuration Consistency Tests', () {
    testWidgets('Table schemas should be consistent',
      ...
      // 테이블 목록 비교
      final prodTables =
          prodSchema?.map((t) => t['table_name'] as String).toSet() ?? {};
      final testTables =
      ...
      for (final tableName in commonTables) {
        final prodTable =
            prodSchema?.firstWhere((t) => t['table_name'] == tableName);
        final testTable =
            testSchema?.firstWhere((t) => t['table_name'] == tableName);

        if (prodTable != null && testTable != null) {
          final prodColumns = (prodTable['columns'] as List?)
                  ?.map((c) => c['column_name'] as String)
                  .toSet() ??
              {};
          final testColumns = (testTable['columns'] as List?)
                  ?.map((c) => c['column_name'] as String)
                  .toSet() ??
              {};
          ...
          for (final columnName in prodColumns.intersection(testColumns)) {
            ...
            final testColumn = (testTable['columns'] as List?)
                ?.firstWhere((c) => c['column_name'] == columnName);
      ...
      expect(deepEq(prodSchema, testSchema), isTrue,
          reason: 'Table schemas differ');
    });

    testWidgets('RLS policies should be consistent',
        (WidgetTester tester) async {
      ...
      expect(prodResponse, isNotNull,
          reason: 'Failed to fetch prod RLS policies');
      expect(testResponse, isNotNull,
          reason: 'Failed to fetch test RLS policies');
      ...
      expect(deepEq(prodPolicies, testPolicies), isTrue,
          reason: 'RLS policies differ');
    });

    testWidgets('Database functions should be consistent',
        (WidgetTester tester) async {
      ...
      prodFunctions
          ?.forEach((func) => (func['argument_types'] as List?)?.sort());
      testFunctions
          ?.forEach((func) => (func['argument_types'] as List?)?.sort());

      expect(deepEq(prodFunctions, testFunctions), isTrue,
          reason: 'Database functions differ');
    });

    testWidgets('Database triggers should be consistent',
        (WidgetTester tester) async {
      ...
      expect(deepEq(prodTriggers, testTriggers), isTrue,
          reason: 'Database triggers differ');
    });
  });
